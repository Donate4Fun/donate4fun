<script>
  import {worker, contentScript, browser, subscribe} from "./common.js";
    ...
    <section class="flex-grow-1">
      <div class="flex-column align-center justify-space-between">
        <div class="flex-row gap-8">
          <span class="font-weight-900 font-20">Donate to <span class="color-blue">{channelTitle}</span></span>
          <img width=24px height=24px src={channelLogo} alt="channel logo">
        </div>
        <div class="flex-row justify-center gap-16">
        {#each amounts as amount_}
          <Button on:click={() => amount = amount_} class="width-120">{toText(amount_)} ⚡</Button>
        {/each}
        </div>
