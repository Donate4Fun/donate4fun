...
<div class="container">
  <div class="main">
    {#if donation.donator_github_account}
    ...
    <a use:link href="/github/{donation.github_user.id}" class="twitter-account">
      <div class="image-name">
        <img alt="avatar" class="avatar" src={donation.github_user.avatar_url}>
        <div class="name">{donation.github_user.name}</div>
      </div>
      <div class="handle">
        <span>@{donation.github_user.login}</span>
  ...
  {#if donation.cancelled_at === null && donation.claimed_at === null}
    <div class="claim">
      <SocialSigninButton colored height=48px width=300px idp="github">Claim with GitHub</SocialSigninButton>
    </div>
  {/if}
</div>
